...
class OuterStrokeTextView2 : AppCompatTextView {
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        if (outerStrokeWidth > 0f) {
            extraPadding = (outerStrokeWidth * dp(0)).toInt()
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // setTextColor() invalidates the view. Suppress only those internal
        // invalidations so drawing a stroke does not schedule another frame.
        isInternalDrawing = true
        try {
            super.setTextColor(outerStrokeColor)
            textPaint.style = Paint.Style.STROKE
            textPaint.strokeWidth = outerStrokeWidth * dp(1)
